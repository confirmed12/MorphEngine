...
namespace ImGui
{
	...
	IMGUI_API void DrawRectRainbow(int x, int y, int width, int height, float flSpeed, float & flRainbow);
    ...
    IMGUI_API void          Columns(int count = 1, const char* id = NULL, bool border = true,bool noresize = false);  // setup number of columns. use an identifier to distinguish multiple column sets. close with Columns(1).
	...
	IMGUI_API bool SliderFloatC(const char * label, float * v, float v_min, float v_max, const char * display_format, float power);
	IMGUI_API bool SliderIntC(const char * label, int * v, int v_min, int v_max, const char * display_format);
...
struct ImColor
{
	...
	static ImColor FromHSB(float hue, float saturation, float brightness)
	{
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddText(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL, bool scaled = false);
...
struct ImFont
{
	...
	IMGUI_API void RenderText(ImDrawList * draw_list, float size, ImVec2 pos, ImU32 col, const ImVec4 & clip_rect, const char * text_begin, const char * text_end, float wrap_width = 0.0f, bool cpu_fine_clip = false, bool scaled = false) const;
