...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        if (g.IO.MouseDrawCursor)
        {
            ...
            g.OverlayDrawList.AddImage(tex_id, pos,             pos + size,             cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(190,0,0,255)); // White fill
...
void ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect, bool tab, bool selected, bool scaled)
{
    // Hide anything after a '##' string
    ...
    if (need_clipping)
    {
        ...
        window->DrawList->AddText(scaled ? Vars.font : g.Font, g.FontSize, pos, GetColorU32(tab ? selected ? ImGuiCol_TabTextActive : ImGuiCol_TabText : ImGuiCol_Text), text, text_display_end, 0.0f, &fine_clip_rect);
    }
    else
    {
        window->DrawList->AddText(scaled ? Vars.font : g.Font, g.FontSize, pos, GetColorU32(tab ? selected ? ImGuiCol_TabTextActive : ImGuiCol_TabText : ImGuiCol_Text), text, text_display_end, 0.0f, NULL);
    }
    if (g.LogEnabled)
        LogRenderedText(pos, text, text_display_end);
}
...
bool ImGui::TextExecute(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool tab, bool selected)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true, -1.0F);
	...
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, /*style.ButtonTextAlign*/ ImVec2(0.f, 0.5f) , &bb, tab, selected,true);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool tab, bool selected)
{
	...
	//window->DrawList->AddTriangleFilled(bb.Min + ImVec2(Vars.Ragebot.HitChanceAmt, Vars.Ragebot.AutoWallDmg), bb.Max + ImVec2(Vars.Ragebot.Antiaim.PCFake, Vars.Ragebot.Antiaim.PCReal), ImVec2(Vars.Ragebot.AimstepAmount, Vars.Ragebot.FOV), col);
...
{
	return TextExecute(label, size_arg, 0, true, selected);
	/*int flags = 0;
...
bool ImGui::SliderBehaviorC(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags, ImVec4 color, const char* format)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    const ImGuiStyle& style = g.Style;
	...
	const float check_sz = ImMin(5, 5);
	const float check_sz2 = check_sz / 2;
	ImVec2* vecs = new ImVec2[3];
	vecs[0] = grab_bb.Min + ImVec2(0, 0);
	vecs[1] = grab_bb.Max - ImVec2(-1, 9);
	vecs[2] = grab_bb.Max - ImVec2(5, 1);
	window->DrawList->AddPolyline(vecs, 3, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), false, 4.f, true);
...
bool ImGui::SliderFloat(const char* text, float* v, float v_min, float v_max, const char* display_format, float power, ImVec2 size, ImVec4 color)
{
    ...
    const bool value_changed = SliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision, 0, color, display_format);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
		...
		//window->DrawList->AddLine(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(2, 2), IM_COL32(220, 220, 220, 255), 1.5f);
		//window->DrawList->AddLine(check_bb.Min + ImVec2(1, 19),check_bb.Max + ImVec2(-1.8, -20), IM_COL32(220, 220, 220, 255), 1.5f);
    }
...
bool ImGui::SliderFloatC(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	// Actual slider behavior + render grab
	const bool value_changed = SliderBehaviorC(frame_bb, id, v, v_min, v_max, power, decimal_precision);
...
bool ImGui::SliderIntC(const char* label, int* v, int v_min, int v_max, const char* display_format)
{
	if (!display_format)
		display_format = "%.0f";
	float v_f = (float)*v;
	bool value_changed = SliderFloatC(label, &v_f, (float)v_min, (float)v_max, display_format, 1.0f);
	*v = (int)v_f;
	return value_changed;
}
    ...
    case ImGuiColorEditMode_HSV:
        {
            ...
            for (int n = 0; n < components; n++)
            {
                ...
                value_changed |= SliderIntC(ids[n], &i[n], 0, 255, fmt[n]); //DragInt(ids[n], &i[n], 1.0f, 0, 255, fmt[n]);
            }
            PopItemWidth();
            PopItemWidth();
        }
...
void ImGui::Columns(int columns_count, const char* id, bool border,bool noresize)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
